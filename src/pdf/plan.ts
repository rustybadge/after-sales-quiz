...
export function buildPlanPdf(results: QuizResults): Uint8Array {
  ...
  doc.text('HUMBLEBEE', 20, 18);
  
  // Subtitle
  doc.setFontSize(16);
  doc.setFont('helvetica', 'normal');
  doc.text('After-Sales Performance Action Plan', 20, 30);
  
  // Company info
  doc.setTextColor(0, 0, 0);
  doc.setFontSize(12);
  doc.text(`Company: ${results.company || 'Not specified'}`, 20, 45);
  doc.text(`Date: ${new Date().toLocaleDateString()}`, 20, 52);
  doc.text(`Overall Score: ${results.totalScore}%`, 20, 59);
  doc.text(`Persona: ${results.personaName}`, 20, 66);
  ...
  doc.text('Generated by Humblebee After-Sales Quiz', 20, pageHeight - 20);
  doc.text('hello@humblebee.se | www.humblebee.se', 20, pageHeight - 15);
